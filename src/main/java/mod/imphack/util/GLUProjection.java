				...
				if ((extrudeInverted && !outsideInvertedFrustum)
					...
					if ((extrudeInverted && !outsideInvertedFrustum)
						...
						if (GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection,
							...
							if (opposite) {
								...
								vecY = this.displayHeight * this.heightScale
										- ((double) displayHeight - (double) this.coords.get(1))
												* (double) this.heightScale
										- this.displayHeight * this.heightScale / 2.0F;
							} else {
								vecX = (double) this.coords.get(0) * this.widthScale
										- this.displayWidth * this.widthScale / 2.0F;
								vecY = ((double) this.displayHeight - (double) this.coords.get(1))
										* (double) this.heightScale - this.displayHeight * this.heightScale / 2.0F;
							}
						} else {
						...
						if (GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection,
							...
							double guiY = ((double) this.displayHeight - (double) this.coords.get(1))
									* (double) this.heightScale;
							if (opposite) {
								// Invert coordinates
